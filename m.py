...
# ======================================================
# إعدادات الفيديو (Video Configuration)
# ======================================================
ALLOWED_VIDEO_EXTENSIONS = {'mp4', 'avi', 'mov', 'webm', 'mkv', 'flv'}
MAX_VIDEO_SIZE = 500 * 1024 * 1024  # 500 MB
VIDEOS_DIR = None  # سيتم تعيينه لاحقاً
...
for d in [STATIC_DIR, VIDEOS_DIR]:
...
def allowed_video_file(filename):
    """التحقق من صيغة الفيديو المدعومة"""
...
# ======================================================
chat_sessions = {}  # ��اكرة المحادثات المؤقتة في الذاكرة
...
# ======================================================
# ✅ رفع الفيديوهات (Upload Videos) - جديد!
# ======================================================
@app.route('/api/v2/chapters/<ch_id>/upload_video', methods=['POST'])
@admin_required
def upload_video(ch_id):
    """رفع ملف فيديو لفصل معين"""
    try:
        # التحقق من وجود الملف
        if 'video_file' not in request.files:
            return jsonify({"status": "error", "message": "لم يتم اختيار ملف فيديو"}), 400
        
        video_file = request.files['video_file']
        video_title = request.form.get('title', 'فيديو بدون عنوان').strip()
        video_duration = request.form.get('duration', '').strip()
        ...
        if not allowed_video_file(video_file.filename):
            return jsonify({
                "status": "error", 
                "message": f"صيغة الملف غير مدعومة. الصيغ المدعومة: {', '.join(ALLOWED_VIDEO_EXTENSIONS)}"
            }), 400
        ...
        if file_size > MAX_VIDEO_SIZE:
            return jsonify({
                "status": "error",
                "message": f"حجم الملف كبير جداً. الحد الأقصى: {MAX_VIDEO_SIZE // (1024*1024)} MB"
            }), 400
        ...
        for t in db['teachers']:
            for ch in t['chapters']:
                if ch['id'] == ch_id:
                    ...
                    video_id = f"vid_{timestamp}_{generate_unique_id().lower()}"
                    ...
                    ch["videos"].append(video_data)
                    save_json(CONTENT_FILE, db)

                    log_action("upload_video", f"title={video_title}, ch_id={ch_id}, size={file_size}", session.get("admin_name"))
        ...
        return jsonify({"status": "error", "message": f"خطأ في الرفع: {str(e)}"}), 500

# ======================================================
# ✅ تحميل / مشاهدة الفيديوهات
# ======================================================
@app.route('/api/v2/videos/<video_id>', methods=['GET'])
            ...
            for ch in t['chapters']:
                for v in ch.get('videos', []):
                    if v.get('id') == video_id:
                        file_path = os.path.join(VIDEOS_DIR, v.get('filename'))
                        if os.path.exists(file_path):
                            return send_from_directory(VIDEOS_DIR, v.get('filename'), as_attachment=False)
                        return jsonify({"status": "error", "message": "ملف الفيديو غير موجود"}), 404
        
        return jsonify({"status": "error", "message": "الفيديو غير موجود"}), 404
...
# ✅ حذف الفيديوهات
# ======================================================
@app.route('/admin/delete_video/<t_id>/<ch_id>/<v_id>')
@admin_required
def delete_video(t_id, ch_id, v_id):
    """حذف فيديو"""
    try:
        db = load_json(CONTENT_FILE, {"teachers": []})
                    ...
                    if ch['id'] == ch_id:
                        for v in ch.get('videos', []):
                            if v['id'] == v_id:
                                ...
                                ch['videos'] = [vid for vid in ch['videos'] if vid['id'] != v_id]
                                save_json(CONTENT_FILE, db)
                                log_action("delete_video", f"v_id={v_id}", session.get("admin_name"))
                                return redirect("/admin?success=video_deleted")
        
        return redirect("/admin?error=video_not_found")

    except Exception as e:
        print(f"[Error] Delete video: {e}")
...
.upload-zone {
  ...
  text-align: center; background: rgba(59,130,246,0.05); transition: all 0.2s;
  cursor: pointer;
}
.upload-zone:hover { border-color: var(--blue); background: rgba(59,130,246,0.1); }
.upload-zone.dragover { border-color: var(--blue); background: rgba(59,130,246,0.15); }
.upload-zone input { display: none; }
.upload-icon { font-size: 3rem; margin-bottom: 12px; }
.upload-text { color: var(--text2); font-size: 0.9rem; margin-bottom: 8px; }
.upload-hint { color: #666; font-size: 0.8rem; }
.progress-bar { width: 100%; height: 6px; background: var(--surface2); border-radius: 3px; margin-top: 12px; overflow: hidden; display: none; }
.progress-fill { height: 100%; background: var(--green); width: 0%; transition: width 0.3s; }

/* Video List */
.video-item {
  display: flex; justify-content: space-between; align-items: center;
  padding: 12px; background: var(--surface2); border-radius: 10px; margin-bottom: 10px;
}
.video-info { flex: 1; }
.video-title { font-weight: 600; color: var(--text); }
.video-meta { font-size: 0.75rem; color: var(--text2); margin-top: 4px; }
        ...
        <div>
          ...
          <div class="teacher-card">
            ...
            <div class="chapter-list">
              {% if t.chapters %}
                {% for ch in t.chapters %}
                <div class="chapter-item" style="flex-direction: column; align-items: flex-start;">
                  <div style="display: flex; justify-content: space-between; width: 100%; align-items: center;">
                    <span>📁 {{ ch.title }}</span>
                    <a href="/admin/delete_chapter/{{ t.id }}/{{ ch.id }}" class="btn btn-danger">حذف الفصل</a>
                  </div>
                  <div style="font-size: 0.75rem; color: var(--text2); margin-top: 8px; width: 100%;">
                    📹 {{ ch.videos|length }} فيديو
                    {% if ch.videos %}
                      <div style="margin-top: 8px;">
                        ...
                        <div class="video-item">
                          <div class="video-info">
                            ...
                            <div class="video-meta">{{ v.size_bytes // (1024*1024) }} MB • {{ v.added_at[:10] }}</div>
                          </div>
                          <a href="/admin/delete_video/{{ t.id }}/{{ ch.id }}/{{ v.id }}" onclick="return confirm('حذف الفيديو؟')" class="btn btn-danger" style="font-size: 0.7rem;">🗑</a>
                        </div>
                        {% endfor %}
                      </div>
                    {% endif %}
                  </div>
                  <form action="/api/v2/chapters/{{ ch.id }}/upload_video" method="POST" enctype="multipart/form-data" style="width: 100%; margin-top: 12px;">
                    <div class="form-group" style="margin-bottom: 8px;">
                      <label>رفع فيديو</label>
                      <input type="file" name="video_file" accept="video/*" required style="padding: 8px;">
                      <input type="text" name="title" placeholder="عنوان الفيديو" required style="margin-top: 8px;">
                    </div>
                    <button type="submit" class="btn btn-success" style="width: 100%;">⬆️ رفع الفيديو</button>
                  </form>
                </div>
                {% endfor %}
              {% else %}
        ...
        <div style="font-size: 0.85rem; line-height: 2;">
          <div style="display: grid; gap: 10px;">
            {% set endpoints = [
              ...
              ('POST', '/api/v2/chapters/:id/upload_video', 'رفع فيديو'),
              ('GET', '/api/v2/videos/:id', 'تحميل فيديو'),
...
if (params.get('success')) {
  ...
  toast.textContent = params.get('success') === 'video_deleted' ? '✅ تم حذف الفيديو!' : '✅ تمت العملية بنجاح!';
...
<p class="subtitle">توثيق نقاط النهاية - الإصدار 2.0 (مع رفع الفيديوهات)</p>

<div class="endpoint">
  <span class="method post">POST</span><span class="path">/api/v2/chapters/:id/upload_video</span>
  <div class="desc">🎬 رفع فيديو جديد لفصل معين - Form Data (multipart)</div>
  <pre>رفع ملف: video_file (mp4, avi, mov, webm, mkv, flv)
الحقول: title (اختياري), duration (اختياري)
الحد الأقصى: 500 MB</pre>
</div>

<div class="endpoint">
  <span class="method get">GET</span><span class="path">/api/v2/videos/:id</span>
  <div class="desc">بث أو تحميل فيديو - يدعم البث المباشر والتحميل</div>
</div>
...
<p style="margin-top: 16px; color: #4b5563; font-size: 0.85rem;">📹 رفع الفيديوهات يتطلب تسجيل الدخول كمسؤول (Admin)</p>
</body>
</html>
"""
...
if __name__ == "__main__":
    ...
    print("📹 رفع الفيديوهات: POST /api/v2/chapters/:id/upload_video")
    print("=" * 50)
